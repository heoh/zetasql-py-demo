"""BigQuery-compatible language options configuration.

Python port of BigQueryLanguageOptions.java from zetasql-toolkit.
...
def get_bigquery_language_options() -> LanguageOptions:
    """Get BigQuery-compatible LanguageOptions.
    
    Configures LanguageOptions with:
    - 80+ BigQuery-specific language features
    - Supported statement kinds (SELECT, INSERT, UPDATE, MERGE, CREATE, DROP, etc.)
    - Reservable keywords (QUALIFY)
    ...
    options.supported_statement_kinds = [
    ...
    # options.enable_reservable_keyword("QUALIFY")
